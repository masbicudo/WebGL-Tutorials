...
function draw(gl, shape, programInfo, uniforms) {
  ...
  for(var pow = 1; pow < maxPow; pow++) {
    ...
    gl.viewport(0, 0, r.x*mul, r.y*mul);
    var t0 = performance.now();
    var cnt = 16*Math.floor(Math.pow(1.42, maxPow-pow));
    for(var it2 = 0; it2 < cnt; it2++) {
      renderScene(gl, shape, programInfo, uniforms, 0);
    }
    var t1 = performance.now();
    var ts = (t1 - t0)/cnt;
    console.log({ size: mul, repeats: cnt, avgTime: ts });
  }
